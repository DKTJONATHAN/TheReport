...
export const POST = async ({ request }) => {
  try {
    ...
    // Ensure GOOGLE_CREDENTIALS is available
    if (!process.env.GOOGLE_CREDENTIALS) {
      return new Response(JSON.stringify({ error: 'Missing GOOGLE_CREDENTIALS environment variable' }), {
        status: 500,
        headers: { 'Content-Type': 'application/json' },
      });
    }
